...
(function ($) {
	$.widget("ui.draggable", $.extend({}, $.ui.mouse, {getHandle:function (e) {
		...
		if (this.cssPosition == "relative") {
			...
			this.offset.relative = {top:p.top - (parseInt(this.helper.css("top"), 10) || 0) + this.scrollTopParent.scrollTop(), left:p.left - (parseInt(this.helper.css("left"), 10) || 0) + this.scrollLeftParent.scrollLeft()};
		} else {
		...
		$.extend(this, {PAGEY_INCLUDES_SCROLL:(this.cssPosition == "absolute" && (!this.scrollTopParent[0].tagName || (/(html|body)/i).test(this.scrollTopParent[0].tagName))), PAGEX_INCLUDES_SCROLL:(this.cssPosition == "absolute" && (!this.scrollLeftParent[0].tagName || (/(html|body)/i).test(this.scrollLeftParent[0].tagName))), OFFSET_PARENT_NOT_SCROLL_PARENT_Y:this.scrollTopParent[0] != this.offsetParent[0] && !(this.scrollTopParent[0] == document && (/(body|html)/i).test(this.offsetParent[0].tagName)), OFFSET_PARENT_NOT_SCROLL_PARENT_X:this.scrollLeftParent[0] != this.offsetParent[0] && !(this.scrollLeftParent[0] == document && (/(body|html)/i).test(this.offsetParent[0].tagName))});
		if (o.containment) {
			this.setContainment();
		}
		...
		var o = this.options;
		var position = {top:(e.pageY - this.offset.click.top - this.offset.relative.top - this.offset.parent.top + (this.cssPosition == "fixed" || this.PAGEY_INCLUDES_SCROLL || this.OFFSET_PARENT_NOT_SCROLL_PARENT_Y ? 0 : this.scrollTopParent.scrollTop()) - (this.cssPosition == "fixed" ? $(document).scrollTop() : 0)), left:(e.pageX - this.offset.click.left - this.offset.relative.left - this.offset.parent.left + (this.cssPosition == "fixed" || this.PAGEX_INCLUDES_SCROLL || this.OFFSET_PARENT_NOT_SCROLL_PARENT_X ? 0 : this.scrollLeftParent.scrollLeft()) - (this.cssPosition == "fixed" ? $(document).scrollLeft() : 0))};
	...
	$.extend($.ui.draggable, {defaults:{appendTo:"parent", axis:false, cancel:":input", delay:0, distance:1, helper:"original", scope:"default", cssNamespace:"ui"}});
		...
		for (var i = inst.snapElements.length - 1; i >= 0; i--) {
			...
			if (!((l - d < x1 && x1 < r + d && t - d < y1 && y1 < b + d) || (l - d < x1 && x1 < r + d && t - d < y2 && y2 < b + d) || (l - d < x2 && x2 < r + d && t - d < y1 && y1 < b + d) || (l - d < x2 && x2 < r + d && t - d < y2 && y2 < b + d))) {
				if (inst.snapElements[i].snapping) {
					(inst.options.snap.release && inst.options.snap.release.call(inst.element, null, $.extend(inst.uiHash(), {snapItem:inst.snapElements[i].item})));
				}
...
(function ($) {
	$.widget("ui.droppable", {_setData:function (key, value) {
		if (key == "accept") {
	...
	$.extend($.ui.droppable, {defaults:{disabled:false, tolerance:"intersect", scope:"default", cssNamespace:"ui"}});
	...
	$.ui.ddmanager = {current:null, droppables:{"default":[]}, prepareOffsets:function (t, e) {
...
(function ($) {
	...
	$.widget("ui.sortable", $.extend({}, $.ui.mouse, {_init:function () {
		...
		if (this.options.tolerance == "pointer" || (this.options.tolerance == "guess" && this.helperProportions[this.floating ? "width" : "height"] > item[this.floating ? "width" : "height"])) {
			...
			if (this.floating) {
				if ((x1 + dxClick) > l && (x1 + dxClick) < l + item.width / 2) {
					return 2;
				}
				if ((x1 + dxClick) > l + item.width / 2 && (x1 + dxClick) < r) {
					return 1;
				}
			} else {
				...
				if (direction == 1 && (y1 + dyClick) < t + height / 2) {
					...
					if (direction == 2 && (y1 + dyClick) > t + height / 2) {
						return 1;
					}
				}
			}
		...
		if (this.domPosition.prev != this.currentItem.prev().not(".ui-sortable-helper")[0] || this.domPosition.parent != this.currentItem.parent()[0]) {
			this._propagate("update", e, null, noPropagation);
		}
...
(function ($) {
	$.widget("ui.accordion", {_init:function () {
		...
		clickHandler.call(this.element[0], {target:findActive(this.options.headers, index)[0]});
	}, destroy:function () {
	...
	function clickHandler(event) {
		...
		var toShow = clicked.next(), toHide = options.active.next(), data = {options:options, newHeader:clickedActive && !options.alwaysOpen ? $([]) : clicked, oldHeader:options.active, newContent:clickedActive && !options.alwaysOpen ? $([]) : toShow, oldContent:toHide}, down = options.headers.index(options.active[0]) > options.headers.index(clicked[0]);
...
(function ($) {
	$.widget("ui.autocomplete", {_init:function () {
		$.extend(this.options, {delay:this.options.url ? $.Autocompleter.defaults.delay : 10, max:!this.options.scroll ? 10 : 150, highlight:this.options.highlight || function (value) {
			return value;
	...
	$.Autocompleter.defaults = {inputClass:"ui-autocomplete-input", resultsClass:"ui-autocomplete-results", loadingClass:"ui-autocomplete-loading", minChars:1, delay:400, matchCase:false, matchSubset:true, matchContains:false, cacheLength:10, max:100, mustMatch:false, extraParams:{}, selectFirst:true, formatItem:function (row) {
		return row[0];
	...
	$.Autocompleter.Select = function (options, input, select, config) {
		var CLASSES = {ACTIVE:"ui-autocomplete-over"};
...
(function ($) {
	$.widget("ui.colorpicker", {_init:function () {
		this.charMin = 65;
		...
		if (typeof o.color == "string") {
			...
			if (o.color.r != undefined && o.color.g != undefined && o.color.b != undefined) {
				this.color = this._RGBToHSB(o.color);
			} else {
				if (o.color.h != undefined && o.color.s != undefined && o.color.b != undefined) {
		...
		this.picker.find(".ui-colorpicker-submit").bind("mouseenter", function (e) {
		...
		if (o.flat) {
			this.picker.css({position:"relative", display:"block"});
		} else {
		...
		this.fields.eq(1).val(rgb.r).end().eq(2).val(rgb.g).end().eq(3).val(rgb.b).end();
		...
		this.selectorIndic.css({left:parseInt(150 * hsb.s / 100, 10), top:parseInt(150 * (100 - hsb.b) / 100, 10)});
		...
		if (target.parentNode.className.indexOf("-hex") > 0) {
			...
			if (target.parentNode.className.indexOf("-hsb") > 0) {
				this.color = col = this._fixHSB({h:parseInt(this.fields.eq(4).val(), 10), s:parseInt(this.fields.eq(5).val(), 10), b:parseInt(this.fields.eq(6).val(), 10)});
				this._fillRGBFields(col);
				this._fillHexFields(col);
			} else {
				this.color = col = this._RGBToHSB(this._fixRGB({r:parseInt(this.fields.eq(1).val(), 10), g:parseInt(this.fields.eq(2).val(), 10), b:parseInt(this.fields.eq(3).val(), 10)}));
				this._fillHexFields(col);
				this._fillHSBFields(col);
			}
		}
		...
		this.currentIncrement = {el:$(e.target).parent().addClass("ui-colorpicker-slider"), max:e.target.parentNode.className.indexOf("-hsb-h") > 0 ? 360 : (e.target.parentNode.className.indexOf("-hsb") > 0 ? 100 : 255), y:e.pageY, field:field, val:parseInt(field.val(), 10)};
		...
		this.currentHue = {y:this.picker.find("div.ui-colorpicker-hue").offset().top};
		this._change.apply(this, [e, this.fields.eq(4).val(parseInt(360 * (150 - Math.max(0, Math.min(150, (e.pageY - this.currentHue.y)))) / 150, 10)).get(0)]);
		var self = this;
		$(document).bind("mouseup.cpSlider", function (e) {
		...
		this._change.apply(this, [e, this.fields.eq(6).val(parseInt(100 * (150 - Math.max(0, Math.min(150, (e.pageY - this.currentSelector.pos.top)))) / 150, 10)).end().eq(5).val(parseInt(100 * (Math.max(0, Math.min(150, (e.pageX - this.currentSelector.pos.left)))) / 150, 10)).get(0)]);
		$(document).bind("mouseup.cpSlider", function (e) {
		...
		var self = this;
		$(document).bind("mousedown.colorpicker", function (e) {
		...
		if ((rgb.r == rgb.g) && (rgb.g == rgb.b)) {
			...
			if (rgb.r >= rgb.g && rgb.g >= rgb.b) {
				...
				if (rgb.g >= rgb.r && rgb.r >= rgb.b) {
					hsb.h = 60 + 60 * (rgb.g - rgb.r) / (rgb.g - rgb.b);
				} else {
					if (rgb.g >= rgb.b && rgb.b >= rgb.r) {
						hsb.h = 120 + 60 * (rgb.b - rgb.r) / (rgb.g - rgb.r);
					} else {
						if (rgb.b >= rgb.g && rgb.g >= rgb.r) {
	...
	$.extend($.ui.colorpicker, {defaults:{eventName:"click", color:"ff0000", flat:false}});
})(jQuery);
(function ($) {
	...
	$.widget("ui.dialog", {_init:function () {
		...
		this.uiDialog.draggable({cancel:".ui-dialog-content", helper:options.dragHelper, handle:".ui-dialog-titlebar", start:function () {
			self._moveToTop();
		...
		this.uiDialog.resizable({cancel:".ui-dialog-content", helper:options.resizeHelper, maxWidth:options.maxWidth, maxHeight:options.maxHeight, minWidth:options.minWidth, minHeight:options.minHeight, start:function () {
		...
		var container = this.uiDialogContainer, titlebar = this.uiDialogTitlebar, content = this.element, tbMargin = (parseInt(content.css("margin-top"), 10) || 0) + (parseInt(content.css("margin-bottom"), 10) || 0), lrMargin = (parseInt(content.css("margin-left"), 10) || 0) + (parseInt(content.css("margin-right"), 10) || 0);
		content.height(container.height() - titlebar.outerHeight() - tbMargin);
		content.width(container.width() - lrMargin);
	}});
	$.extend($.ui.dialog, {defaults:{autoOpen:true, autoResize:true, bgiframe:false, buttons:{}, closeOnEscape:true, draggable:true, height:200, minHeight:100, minWidth:150, modal:false, overlay:{}, position:"center", resizable:true, stack:true, width:300, zIndex:1000}, getter:"isOpen", uuid:0, getTitleId:function ($el) {
...
(function ($) {
	...
	$.widget("ui.slider", {plugins:{}, ui:function (e) {
		return {options:this.options, handle:this.currentHandle, value:this.options.axis != "both" || !this.options.axis ? Math.round(this.value(null, this.options.axis == "vertical" ? "y" : "x")) : {x:Math.round(this.value(null, "x")), y:Math.round(this.value(null, "y"))}, range:this._getRange()};
	}, _propagate:function (n, e) {
		$.ui.plugin.call(this, n, [e, this.ui()]);
		...
		$.extend(handleclass.prototype, $.ui.mouse, {_mouseStart:function (e) {
		...
		$(this.handle).each(function () {
			...
			if (!self.options.noKeyboard) {
				return self._keydown(e.keyCode, this.firstChild);
			}
		});
		...
		if (/(33|34|35|36|37|38|39|40)/.test(k)) {
			...
			this.moveTo({x:xpos, y:ypos}, handle);
			return false;
		}
		return true;
		...
		this.offset = this.element.offset();
		this.moveTo({y:this._convertValue(e.pageY - this.offset.top - this.currentHandle[0].offsetHeight / 2, "y"), x:this._convertValue(e.pageX - this.offset.left - this.currentHandle[0].offsetWidth / 2, "x")}, null, !this.options.distance);
		...
		if (this.options.handles) {
			...
			if (value < this._translateValue(handle.min, axis)) {
				...
				if (value > this._translateValue(handle.max, axis)) {
		...
		if (o.axis != "vertical") {
			this.currentHandle.css({left:position.left});
		}
		if (o.axis != "horizontal") {
			this.currentHandle.css({top:position.top});
		}
		this.currentHandle.data("mouse").sliderValue = {x:Math.round(this._convertValue(position.left, "x")) || 0, y:Math.round(this._convertValue(position.top, "y")) || 0};
		if (this.rangeElement) {
			this._updateRange();
		}
		...
		this.currentHandle.data("mouse").sliderValue = {x:Math.round(this._convertValue(x, "x")) || 0, y:Math.round(this._convertValue(y, "y")) || 0};
		if (!noPropagation) {
...
(function ($) {
	$.widget("ui.tabs", {_init:function () {
		...
		return {options:this.options, tab:tab, panel:panel, index:this.$tabs.index(tab)};
		...
		this.$tabs.unbind(".tabs").bind(o.event, function () {
			...
			if (($li.hasClass(o.selectedClass) && !o.unselect) || $li.hasClass(o.disabledClass) || $(this).hasClass(o.loadingClass) || self._trigger("select", null, self.ui(this, $show[0])) === false) {
				this.blur();
				return false;
			}
...
(function ($) {
	...
	$.extend(Datepicker.prototype, {markerClassName:"hasDatepicker", log:function () {
		...
		if ($.datepicker._datepickerShowing) {
			switch (e.keyCode) {
			  ...
			  case 27:
				...
				break;
			  case 33:
				$.datepicker._adjustDate(e.target, (e.ctrlKey ? -$.datepicker._get(inst, "stepBigMonths") : -$.datepicker._get(inst, "stepMonths")), "M");
				break;
			  case 34:
				$.datepicker._adjustDate(e.target, (e.ctrlKey ? +$.datepicker._get(inst, "stepBigMonths") : +$.datepicker._get(inst, "stepMonths")), "M");
				break;
		...
		var startDate = (!inst.currentYear || (inst.input && inst.input.val() == "") ? null : new Date(inst.currentYear, inst.currentMonth, inst.currentDay));
		if (this._get(inst, "rangeSelect")) {
		...
		var newMinDate = (!inst.rangeStart ? null : new Date(inst.selectedYear, inst.selectedMonth, inst.selectedDay));
...
(function ($) {
	var counter = 0;
	$.widget("ui.magnifier", {_init:function () {
		var self = this, o = this.options;
		this.element.addClass("ui-magnifier").bind("click.magnifier", function (e) {
			(!self.disabled && o.click && o.click.apply(this, [e, {options:self.options, current:self.current[0], currentOffset:self.current[1]}]));
		});
		...
		this.element.find(o.items).each(function () {
			var $this = $(this);
			self.items.push([this, $this.offset(), [$this.width(), $this.height()], (o.overlap ? $this.position() : null)]);
			(o.opacity && $this.css("opacity", o.opacity.min));
		});
		(o.overlap && $.each(this.items, function () {
			$(this[0]).css({position:"absolute", top:this[3].top, left:this[3].left});
		}));
		...
		this.reset();
		this.element.removeClass("ui-magnifier ui-magnifier-disabled").unbind(".magnifier");
		...
		$.each(this.items, function () {
			var item = this;
			$(item[0]).css({width:item[2][0], height:item[2][1], top:(item[3] ? item[3].top : 0), left:(item[3] ? item[3].left : 0)});
		...
		for (var i = 0; i < this.items.length; i++) {
			...
			if (!o.axis) {
				distance = Math.sqrt(Math.pow(p[0] - ((c[3] ? this.pp.left : c[1].left) + parseInt(c[0].style.left, 10)) - (c[0].offsetWidth / 2), 2) + Math.pow(p[1] - ((c[3] ? this.pp.top : c[1].top) + parseInt(c[0].style.top, 10)) - (c[0].offsetHeight / 2), 2));
			} else {
				if (o.axis == "y") {
			...
			if (distance < o.distance) {
				...
				if (!o.axis || o.axis != "y") {
					$(c[0]).css({width:c[2][0] + (c[2][0] * (o.magnification - 1)) - (((distance / o.distance) * c[2][0]) * (o.magnification - 1)), left:(c[3] ? (c[3].left + o.verticalLine * ((c[2][1] * (o.magnification - 1)) - (((distance / o.distance) * c[2][1]) * (o.magnification - 1)))) : 0)});
				}
				if (!o.axis || o.axis != "x") {
					$(c[0]).css({height:c[2][1] + (c[2][1] * (o.magnification - 1)) - (((distance / o.distance) * c[2][1]) * (o.magnification - 1)), top:(c[3] ? c[3].top : 0) + (o.baseline - 0.5) * ((c[2][0] * (o.magnification - 1)) - (((distance / o.distance) * c[2][0]) * (o.magnification - 1)))});
				}
				if (o.opacity) {
					$(c[0]).css("opacity", o.opacity.max - (distance / o.distance) < o.opacity.min ? o.opacity.min : o.opacity.max - (distance / o.distance));
				}
			} else {
				$(c[0]).css({width:c[2][0], height:c[2][1], top:(c[3] ? c[3].top : 0), left:(c[3] ? c[3].left : 0)});
	...
	$.extend($.ui.magnifier, {defaults:{distance:150, magnification:2, baseline:0, verticalLine:-0.5, items:"> *"}});
})(jQuery);
(function ($) {
	$.widget("ui.progressbar", {_init:function () {
		this._interval = this.options.interval;
		var self = this, options = this.options, id = (new Date()).getTime() + Math.random(), text = options.text || "0%";
		this.element.addClass("ui-progressbar").width(options.width);
		$.extend(this, {active:false, pixelState:0, percentState:0, identifier:id, bar:$("<div class=\"ui-progressbar-bar ui-hidden\"></div>").css({width:"0px", overflow:"hidden", zIndex:100}), textElement:$("<div class=\"ui-progressbar-text\"></div>").html(text).css({width:"0px", overflow:"hidden"}), textBg:$("<div class=\"ui-progressbar-text ui-progressbar-text-back\"></div>").html(text).css({width:this.element.width()}), wrapper:$("<div class=\"ui-progressbar-wrap\"></div>")});
		this.wrapper.append(this.bar.append(this.textElement.addClass(options.textClass)), this.textBg).appendTo(this.element);
	}, plugins:{}, ui:function (e) {
		return {instance:this, identifier:this.identifier, options:this.options, element:this.bar, textElement:this.textElement, pixelState:this.pixelState, percentState:this.percentState};
	}, _propagate:function (n, e) {
		$.ui.plugin.call(this, n, [e, this.ui()]);
		...
		jQuery.easing[this.identifier] = function (x, t, b, c, d) {
			var inc = options.increment, width = options.width, step = ((inc > width ? width : inc) / width), state = Math.round(x / step) * step;
		...
		var self = this, options = this.options, interval = options.interval;
		this.bar.animate({width:options.width}, {duration:interval, easing:this.identifier, step:function (step, b) {
	...
	$.ui.progressbar.defaults = {width:300, duration:3000, interval:200, increment:1, range:true, text:"", addClass:"", textClass:""};
})(jQuery);
(function ($) {
	$.widget("ui.spinner", {_init:function () {
		...
		if (this._items > 1) {
			this.element.addClass("ui-spinner-list").css("height", this.element.outerHeight() / this._items).children().addClass("ui-spinner-listitem").end().parent().css("height", this.element.outerHeight()).end();
		...
		return (e.keyCode == KEYS.TAB || e.keyCode == KEYS.BACKSPACE || e.keyCode == KEYS.LEFT || e.keyCode == KEYS.RIGHT || e.keyCode == KEYS.PERIOD || e.keyCode == KEYS.NUMPAD_DECIMAL || e.keyCode == KEYS.NUMPAD_SUBTRACT || (e.keyCode >= 96 && e.keyCode <= 105) || (/[0-9\-\.]/).test(String.fromCharCode(e.keyCode))) ? true : false;
		...
		if (this.element.hasClass("ui-spinner-list") && ((d == "up" && this._getValue() <= this.options.max) || (d == "down" && this._getValue() >= this.options.min))) {
			this.element.animate({marginTop:"-" + this._getValue() * this.element.outerHeight()}, {duration:"fast", queue:false});
		}
		...
		return {options:this.options, element:this.element, value:this._getValue(), add:this._addItem};
	}, _propagate:function (n, e) {
		$.ui.plugin.call(this, n, [e, this.ui()]);
	...
	$.extend($.ui.spinner, {defaults:{stepping:1, start:0, incremental:true, currency:false}, format:{number:function (num, dec) {
		return this.round(num, dec);
	}, currency:function (num, sym) {
		return (num !== Math.abs(num) ? "-" : "") + sym + this.round(Math.abs(num), 2);
	}, round:function (num, dec) {
		var s = Math.round(parseFloat(num) * Math.pow(10, dec)) / Math.pow(10, dec);
		if (dec > 0) {
...
(function ($) {
	...
	$.fn.extend({_show:$.fn.show, _hide:$.fn.hide, __toggle:$.fn.toggle, _addClass:$.fn.addClass, _removeClass:$.fn.removeClass, _toggleClass:$.fn.toggleClass, effect:function (fx, o, speed, callback) {
		...
		return this.queue(function () {
			...
			for (var i = 0; i < rows; i++) {
				for (var j = 0; j < cells; j++) {
					el.clone().appendTo("body").wrap("<div></div>").css({position:"absolute", visibility:"visible", left:-j * (width / cells), top:-i * (height / rows)}).parent().addClass("effects-explode").css({position:"absolute", overflow:"hidden", width:width / cells, height:height / rows, left:offset.left + j * (width / cells) + (o.options.mode == "show" ? (j - Math.floor(cells / 2)) * (width / cells) : 0), top:offset.top + i * (height / rows) + (o.options.mode == "show" ? (i - Math.floor(rows / 2)) * (height / rows) : 0), opacity:o.options.mode == "show" ? 0 : 1}).animate({left:offset.left + j * (width / cells) + (o.options.mode == "show" ? 0 : (j - Math.floor(cells / 2)) * (width / cells)), top:offset.top + i * (height / rows) + (o.options.mode == "show" ? 0 : (i - Math.floor(rows / 2)) * (height / rows)), opacity:o.options.mode == "show" ? 1 : 0}, o.duration || 500);
				}
			}
			setTimeout(function () {
...
(function ($) {
	$.effects.fold = function (o) {
		return this.queue(function () {
			...
			if (percent) {
				size = parseInt(percent[1]) / 100 * distance[mode == "hide" ? 0 : 1];
			}
			if (mode == "show") {
